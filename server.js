...
try {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS scripts (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, code TEXT, author TEXT, category TEXT DEFAULT 'GUI', created_at DATETIME DEFAULT CURRENT_TIMESTAMP);
  `);
  // Tabellenspalten absichern, falls Spalte 'category' noch fehlte
  try { db.exec(`ALTER TABLE scripts ADD COLUMN category TEXT DEFAULT 'GUI';`); } catch(e){}
  console.log("Datenbank erfolgreich verbunden & aktualisiert!");
} catch (err) {
...
app.get('/api/auth/google', (req, res, next) => {
  if (!process.env.GOOGLE_CLIENT_ID) return res.status(500).send("Google Client ID fehlt!");
...
app.get('/api/auth/github', (req, res, next) => {
  if (!process.env.GITHUB_CLIENT_ID) return res.status(500).send("GitHub Client ID fehlt!");
...
app.post('/api/scripts', (req, res) => {
  const { title, code, author, category } = req.body;
  if (!title || !code) return res.status(400).json({ error: "Titel und Code dürfen nicht leer sein." });

  const cat = category || 'GUI';
  const stmt = db.prepare('INSERT INTO scripts (title, code, author, category) VALUES (?, ?, ?, ?)');
  stmt.run(title, code, author || 'Anonym', cat);
...
// Skripte eines spezifischen Nutzers abrufen (Profilansicht)
app.get('/api/user/:username/scripts', (req, res) => {
  const stmt = db.prepare('SELECT * FROM scripts WHERE author = ? ORDER BY id DESC');
  const scripts = stmt.all(req.params.username);
  res.json(scripts);
});
